...
namespace Montreal.Core.Crosscutting.Common.Extensions
{
    public static class ClaimsPrincipalExtension
    {
        public static Guid GetUserIdFromToken(this ClaimsPrincipal claimsPrincipal)
        {
            ...
            {
                var claims = claimsPrincipal.FindAll(t => t.Type == "userid");
        ...
        public static Guid GetIdentityIdFromToken(this ClaimsPrincipal claimsPrincipal)
        {
            var checkClaimsPrincipal = claimsPrincipal.CheckClaimsPrincipal();

            if (checkClaimsPrincipal)
            {
                var claims = claimsPrincipal.FindAll(t => t.Type == "identityId");
        ...
        public static Guid GetOrganizationFromToken(this ClaimsPrincipal claimsPrincipal)
        {
            var checkClaimsPrincipal = claimsPrincipal.CheckClaimsPrincipal();

            if (checkClaimsPrincipal)
            {
                var claims = claimsPrincipal.FindAll(t => t.Type == "organizationId");
